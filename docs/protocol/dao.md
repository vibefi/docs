...
- The fee splitter is linked to [`DappRegistry`](../components/contracts.md) so distribution can follow currently approved vapp metadata.
...
- The [client](../components/client.md) checks whether the connected wallet holds a valid subscription NFT.
...
- Proposing governance actions requires minimum delegated [`VFI`](./vfi.md) voting power (current default: `100` BPS / `1%` via `MinimumDelegationRequirement`).
